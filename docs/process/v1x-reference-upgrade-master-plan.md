...
- Repair old helper assumptions without reverting the staged workflow.
...
- Passed on 2026-05-26: `237 passed`.
- Evidence capture now explicitly advances through draft-ready, submit-review, optional revision, approval, and execution.
...
- Implement `team task list`, `team task next`, and `team task done` without adding a second task database.
- Prefer existing checklist/work-graph state as the task pool source of truth.
...
- Add operator override support while keeping `resume_if_same_task` as the normal execution default.
- Include context policy, source session, resume target, and stop reason in execution metadata and summaries.
...
- Passed on 2026-05-26: `157 passed`.
- Execution metadata and inspection summaries now expose `fresh`, `resume`, and `resume_if_same_task` context policy.
...
- Keep existing message storage intact while adding thread labels for main, review, and rescue handoffs.
- Add `team inspect-knowledge` for decisions and lessons generated by the current session.
...
- Passed on 2026-05-26: `132 passed`.
- Messages now expose main/review thread labels and session knowledge artifacts persist decisions and lessons.
...
- Passed on 2026-05-26: `108 passed`.
- Status, UI, and evidence surfaces now expose approval state, intervention reason, runtime health, and usage/cost placeholders.
